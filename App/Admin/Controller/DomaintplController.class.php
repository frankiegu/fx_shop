...
class DomaintplController extends BaseController{
    public function _initialize() {
        parent::_initialize();
        $this->tbname = 'tpl_domain';
    }
    //
    ...
    protected function _after_add($id){
        $log = [];
        $row = M($this->tbname)->where(['tdid'=>$id])->find();
        $log['happen'] = '添加了模板名为"'.$row['tname'].'的域名模板信息';
    ...
    protected function _after_save($data){
        $tdid = $data['tdid'];
        S('domain_tpl_'.$tdid, null);
        $log = [];
        $log['happen'] = '修改了模板名为"'.$data['tname'].'"的域名模板信息';
